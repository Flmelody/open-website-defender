...
`Website Defender` 为暴露在公网的网站提供统一认证、智能威胁检测和多层访问控制。无需安装 Agent，无需云订阅 -- 只需一个可执行文件配合 Nginx 即可运行。
...
`Website Defender` 通过提供自托管的安全网关、统一鉴权和访问控制策略来解决这些问题 -- 对现有应用**无需任何代码修改**。
...
授权域实现了多租户访问控制，允许不同用户通过同一个 Defender 实例访问不同的受保护服务。
