...
- **授权域管理**：集中管理受保护的域名资源。作为 IP 白名单域名绑定和用户访问作用域的统一数据源。删除授权域时级联清理关联的 IP 白名单条目和用户作用域引用。
- **授权域访问控制**：通过逗号分隔的授权域模式限制用户可访问的域名（如 `gitea.com, *.internal.org`）。作用域为空则不限制访问。管理员用户跳过作用域检查。域名通过 `X-Forwarded-Host` 请求头获取，回退到 `Host` 请求头。
...
- 集中注册和管理所有受保护的域名
- 作为 IP 白名单域名绑定和用户作用域选择的数据源
- **级联删除**：删除授权域时自动清理关联的 IP 白名单条目和用户作用域引用
