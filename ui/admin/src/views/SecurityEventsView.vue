<template>
  <div class="security-events-view">
    <div class="stats-row">
      <div class="stat-card glass-panel" v-for="stat in statsCards" :key="stat.label">
    ...
    <!-- Top Threat IPs -->
    <div class="terminal-card glass-panel" v-if="stats?.top_ips?.length" style="margin-bottom: 20px;">
      ...
      <div class="data-grid">
        <el-table :data="stats.top_ips" style="width: 100%" class="hacker-table">
          <el-table-column prop="client_ip" :label="t('security_events.client_ip')" width="200">
            <template #default="scope">
              <span class="bright-text">{{ scope.row.client_ip }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="count" :label="t('security_events.event_count')" width="140">
            <template #default="scope">
              <span class="dim-text">{{ scope.row.count }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="threat_score" :label="t('security_events.threat_score')">
            <template #default="scope">
              <div class="score-bar-wrapper">
                <div class="score-bar" :style="{ width: Math.min(scope.row.threat_score, 100) + '%', background: scoreColor(scope.row.threat_score) }"></div>
                <span class="score-text" :style="{ color: scoreColor(scope.row.threat_score) }">{{ scope.row.threat_score }}</span>
              </div>
            </template>
          </el-table-column>
        </el-table>
    ...
    <div class="terminal-card glass-panel">
      <div class="card-header no-select">
        ...
        <div class="header-right">
          <el-select v-model="filters.event_type" :placeholder="t('security_events.filter_type')" clearable size="small" style="width: 160px; margin-right: 8px;">
            <el-option :label="t('security_events.type_auto_ban')" value="auto_ban" />
            <el-option :label="t('security_events.type_brute_force')" value="brute_force" />
            <el-option :label="t('security_events.type_scan_detected')" value="scan_detected" />
            <el-option :label="t('security_events.type_js_challenge_fail')" value="js_challenge_fail" />
          </el-select>
          <el-input v-model="filters.client_ip" :placeholder="t('security_events.filter_ip')" clearable size="small" style="width: 150px; margin-right: 8px;" @keyup.enter="fetchData" />
      ...
      <div class="data-grid">
        <el-table :data="tableData" v-loading="loading" style="width: 100%" class="hacker-table">
          ...
          <el-table-column prop="event_type" :label="t('security_events.event_type')" width="160">
            <template #default="scope">
              <el-tag :type="eventTypeColor(scope.row.event_type)" size="small" effect="dark">
                {{ eventTypeLabel(scope.row.event_type) }}
              </el-tag>
            </template>
          </el-table-column>
          <el-table-column prop="client_ip" :label="t('security_events.client_ip')" width="160">
            <template #default="scope">
              <span class="bright-text">{{ scope.row.client_ip }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="created_at" :label="t('security_events.timestamp')" width="180">
            <template #default="scope">
...
const statsCards = computed(() => [
  { label: t('security_events.stats_total'), value: stats.value?.total_events ?? 0 },
  { label: t('security_events.stats_auto_bans'), value: stats.value?.auto_bans_24h ?? 0 },
  { label: t('security_events.stats_threats'), value: stats.value?.top_ips?.length ?? 0 },
])
...
const eventTypeColor = (type: string) => {
  switch (type) {
    case 'auto_ban': return 'danger'
    case 'brute_force': return 'warning'
    case 'scan_detected': return 'info'
    case 'js_challenge_fail': return ''
    default: return 'info'
  }
}
...
const eventTypeLabel = (type: string) => {
  switch (type) {
    case 'auto_ban': return t('security_events.type_auto_ban')
    case 'brute_force': return t('security_events.type_brute_force')
    case 'scan_detected': return t('security_events.type_scan_detected')
    case 'js_challenge_fail': return t('security_events.type_js_challenge_fail')
    default: return type
  }
}
...
<style scoped>
.security-events-view { width: 100%; }
.stats-row { display: flex; gap: 16px; margin-bottom: 20px; }
.stat-card { flex: 1; padding: 20px; text-align: center; }
.stat-value { font-size: 28px; font-weight: bold; color: #0f0; font-family: 'Courier New', monospace; text-shadow: 0 0 10px rgba(0, 255, 0, 0.3); }
.stat-label { font-size: 12px; color: #8a8; margin-top: 6px; font-family: 'Courier New', monospace; text-transform: uppercase; }
