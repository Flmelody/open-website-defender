    ...
    <div class="terminal-card glass-panel">
      <div class="card-header no-select">
        ...
        <div class="header-right">
          <el-button type="primary" size="small" @click="handleAdd">{{ t('authorized_domain.new_domain') }}</el-button>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle.toUpperCase()" width="500px" destroy-on-close>
      <el-form ref="formRef" :model="form" :rules="rules" label-position="top" class="hacker-form">
        <el-form-item :label="'> ' + t('authorized_domain.name')" prop="name">
          <el-input v-model="form.name" :placeholder="t('authorized_domain.name_placeholder')" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const fetchData = async () => {
  loading.value = true
  try {
    const res: any = await request.get('/authorized-domains', { params: queryParams })
...
<style scoped>
.authorized-domain-view { width: 100%; }
