<template>
  <div class="oauth-client-view">
    <div class="terminal-card glass-panel">
      <div class="card-header no-select">
        ...
        <div class="header-right">
          <el-button type="primary" size="small" @click="handleAdd">{{ t('oauth.new_client') }}</el-button>
      ...
      <div class="data-grid">
        <el-table :data="tableData" v-loading="loading" style="width: 100%" class="hacker-table">
          ...
          <el-table-column prop="client_id" :label="t('oauth.client_id')" width="200">
            <template #default="scope">
              <span class="dim-text mono-text">{{ scope.row.client_id.substring(0, 16) }}...</span>
            </template>
          </el-table-column>
          <el-table-column prop="trusted" :label="t('oauth.trusted')" width="100">
            <template #default="scope">
              <el-tag :type="scope.row.trusted ? 'warning' : 'info'" size="small" effect="dark">
    ...
    <el-dialog v-model="dialogVisible" :title="(isEdit ? t('oauth.title_edit') : t('oauth.title_create')).toUpperCase()" width="600px" destroy-on-close>
      <el-form ref="formRef" :model="form" :rules="rules" label-position="top" class="hacker-form">
        <el-form-item :label="'> ' + t('oauth.name')" prop="name">
          <el-input v-model="form.name" :placeholder="t('oauth.name_placeholder')" />
        </el-form-item>
        <el-form-item :label="'> ' + t('oauth.redirect_uris')" prop="redirect_uris_text">
          <el-input v-model="form.redirect_uris_text" type="textarea" :rows="3"
            :placeholder="t('oauth.redirect_uris_placeholder')" />
          <div class="field-hint">{{ t('oauth.redirect_uris_hint') }}</div>
        </el-form-item>
        <el-form-item :label="'> ' + t('oauth.scopes')">
          <el-input v-model="form.scopes" placeholder="openid profile email" />
        </el-form-item>
        <el-form-item>
          <el-checkbox v-model="form.trusted">{{ t('oauth.trusted_label') }}</el-checkbox>
        </el-form-item>
        <el-form-item v-if="isEdit">
          <el-checkbox v-model="form.active">{{ t('oauth.active_label') }}</el-checkbox>
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    >
      <div class="token-result">
        ...
        <div class="info-row">
          ...
          <el-button type="primary" size="small" class="copy-btn" @click="copyText(createdClient.client_id)">
            <el-icon><CopyDocument /></el-icon>
          </el-button>
        </div>
        <div class="info-row">
          <span class="info-label">Client Secret:</span>
          <code class="info-value highlight">{{ createdClient.client_secret }}</code>
          <el-button type="primary" size="small" class="copy-btn" @click="copyText(createdClient.client_secret)">
            <el-icon><CopyDocument /></el-icon>
          </el-button>
        </div>
      </div>
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="closeSecretDialog">{{ t('oauth.understood') }}</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
...
const createdClient = reactive<CreatedClient>({ client_id: '', client_secret: '' })

const rules = computed(() => ({
  name: [{ required: true, message: t('login.required'), trigger: 'blur' }],
  redirect_uris_text: [{ required: true, message: t('oauth.redirect_uris_required'), trigger: 'blur' }]
}))

const fetchData = async () => {
  loading.value = true
  try {
    const res: any = await request.get('/oauth-clients', { params: queryParams })
...
.field-hint { color: #8a8; font-size: 12px; margin-top: 4px; font-family: 'Courier New', monospace; }

/* Secret result dialog styles */
.token-result { font-family: 'Courier New', monospace; }
.token-warning {
  display: flex; align-items: center; gap: 8px; padding: 12px 16px;
  background: rgba(255, 165, 0, 0.15); border: 1px solid rgba(255, 165, 0, 0.4);
  border-radius: 4px; color: #ffa500; font-size: 13px; margin-bottom: 20px;
}
.warning-icon { font-size: 18px; flex-shrink: 0; }
.info-row {
  display: flex; align-items: center; gap: 12px; padding: 10px 16px;
  background: rgba(0, 40, 0, 0.6); border: 1px solid #005000; border-radius: 4px; margin-bottom: 8px;
}
.info-label { color: #8a8; white-space: nowrap; min-width: 100px; }
.info-value { flex: 1; color: #0F0; font-size: 13px; word-break: break-all; line-height: 1.5; text-shadow: 0 0 5px rgba(0, 255, 0, 0.2); }
.info-value.highlight { color: #ff0; text-shadow: 0 0 5px rgba(255, 255, 0, 0.3); }
