...
Castellum 内置了 Web 应用防火墙（WAF），提供基于正则表达式的请求过滤功能。WAF 会检查以下请求内容：
...
Castellum 提供 9 条开箱即用的内置规则，覆盖常见的 Web 攻击类型：
...
除了基于正则表达式的模式匹配，Castellum 还内置了**语义分析引擎**，能够理解 SQL 和 HTML 的结构，而非仅依赖字符串模式匹配。该引擎提供了更深层的检测能力，可以捕获绕过正则规则的攻击，同时显著降低误报率。
