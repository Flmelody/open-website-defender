...
export const useAuthStore = defineStore('auth', () => {
  ...
  async function login(username: string, password: string): Promise<{ requires2FA: boolean }> {
    try {
      const res = await request.post<any, AdminLoginResponse>('/admin-login', { username, password })
  ...
  return { token, user, isLoggedIn, pendingChallengeToken, requires2FA, login, verify2FA, cancelChallenge, logout }
})
